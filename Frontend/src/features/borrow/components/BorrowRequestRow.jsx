...
export default function BorrowRequestRow({
  ...
  return (
    <tr style={styles.row}>
      <td style={styles.td} data-label="Book Name">{borrow.book.title}</td>

      <td style={styles.td} data-label="Requested At">
        {new Date(borrow.requestedAt).toLocaleDateString("en-GB")}
      </td>
      

      <td style={styles.td} data-label="Duration">{borrow.availabilityDate.duration}</td>

      <td style={styles.td} data-label="Reader">{borrow.reader.name}</td>
